...
def set_args():
    ...
    parser.add_argument(
        "--train_file_path",
        default="../train_data/train_data2.json",
        type=str,
        help="新闻标题生成的训练数据",
    )
    parser.add_argument(
        "--test_file_path",
        default="../train_data/test_data2.json",
        type=str,
        help="新闻标题生成的测试数据",
    )
    parser.add_argument(
        "--pretrained_model_path",
        default="../models/original_model/",
        type=str,
        help="预训练的GPT2模型的路径",
    )
    parser.add_argument(
        "--data_dir", default="../data_cache", type=str, help="生成缓存数据的存放路径"
    )
    parser.add_argument("--num_train_epochs", default=32, type=int, help="模型训练的轮数")
    parser.add_argument(
        "--train_batch_size", default=16, type=int, help="训练时每个batch的大小"
    )
    ...
    parser.add_argument(
        ...
        default=0.1,
        type=float,
        help="warm up概率，即训练总步长的百分之多少，进行warm up",
    )
    parser.add_argument(
        "--adam_epsilon", default=1e-8, type=float, help="Adam优化器的epsilon值"
    )
    ...
    parser.add_argument(
        "--output_dir", default="../output_models/", type=str, help="模型输出路径"
    )
    parser.add_argument("--seed", type=int, default=2020, help="随机种子")
    parser.add_argument(
        "--max_len", type=int, default=512, help="输入模型的最大长度，要比config中n_ctx小"
    )
    parser.add_argument(
        "--title_max_len", type=int, default=32, help="生成标题的最大长度，要比max_len小"
    )
